...
import { ElementGroupRow, Warning } from '@/lib/element-group-loader/types';

interface ElementGroupTableProps {
  rows: ElementGroupRow[];
  setRows: (rows: ElementGroupRow[]) => void;
...
export default function ElementGroupTable({
  rows,
  setRows,
  ...
  function updateRow(index: number, field: keyof ElementGroupRow, value: string) {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 text-left">
              ...
              <th className="px-3 py-2 font-medium text-gray-600">Element Group Name</th>
              <th className="px-3 py-2 font-medium text-gray-600">Element Name</th>
              <th className="px-3 py-2 font-medium text-gray-600 w-40">Inclusion Status</th>
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2">
                    <input
                      type="text"
                      value={row.elementGroupName}
                      onChange={(e) => updateRow(i, 'elementGroupName', e.target.value)}
                      className={`input-field w-full ${!row.elementGroupName ? 'border-amber-300' : ''}`}
                      placeholder="Element Group Name"
                    />
                  </td>
                  <td className="px-3 py-2">
                    <input
                  ...
                  <td className="px-3 py-2">
                    <select
                      value={row.includeOrExclude}
                      onChange={(e) => updateRow(i, 'includeOrExclude', e.target.value)}
